...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, planes, stride=1,gn_params=2,batchn=False):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        if(batchn):
            self.bn1 = nn.BatchNorm2d(planes)
        else:
            self.bn1 = nn.GroupNorm(gn_params,planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        if(batchn):
            self.bn2 = nn.BatchNorm2d(planes)
        else:
            self.bn2 = nn.GroupNorm(gn_params,planes)

        self.shortcut = nn.Sequential()
        if(batchn):
            norm = nn.BatchNorm2d(planes)
        else:
            norm = nn.GroupNorm(gn_params,planes)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, feature_maps, input_shape, num_classes, batchn):
        ...
        if(batchn):
            ...
            self.bn1 = nn.GroupNorm(gn_params,feature_maps)

        layers = []
        for i, nb in enumerate(num_blocks):
            layers.append(self._make_layer(block, (2 ** i) * feature_maps, nb,stride = 1 if i == 0 else 2,gn_params=gn_params,batchn=batchn))
        self.layers = nn.Sequential(*layers)
        self.features_dim = (2 ** i)*feature_maps
    ...
    def _make_layer(self, block, planes, num_blocks, stride,gn_params,batchn=False):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for i in range(len(strides)):
            stride = strides[i]
            layers.append(block(self.in_planes, planes, stride,gn_params,batchn))
            if i < len(strides) - 1:
...
def ResNet32(feature_maps, input_shape, num_classes, batchn=False):
    return ResNet(BasicBlock, [5, 5, 5], feature_maps, input_shape, num_classes, batchn)